...
trivy-scan:
	...
	@mkdir -p $(REPORTS_DIR)
	@echo "--- [TRIVY] Construindo imagens para análise ---"
	@# Localização dinâmica com verificação
	@USER_DOCKER=$$(find . -maxdepth 3 -iname "*Docker*user*" | head -n 1); \
	PROD_DOCKER=$$(find . -maxdepth 3 -iname "*Docker*product*" | head -n 1); \
	if [ -z "$$USER_DOCKER" ] || [ -z "$$PROD_DOCKER" ]; then \
		echo "Erro: Dockerfiles não encontrados!"; exit 1; \
	fi; \
	docker build -f $$USER_DOCKER -t users-service:latest . && \
	docker build -f $$PROD_DOCKER -t products-service:latest .
	...
	@echo "--- [TRIVY] Iniciando scan de vulnerabilidades ---"
	trivy image --severity HIGH,CRITICAL --input $(REPORTS_DIR)/users_temp.tar > $(REPORTS_DIR)/trivy_users_report.txt 2>&1
	trivy image --severity HIGH,CRITICAL --input $(REPORTS_DIR)/products_temp.tar > $(REPORTS_DIR)/trivy_products_report.txt 2>&1
	@echo "--- [TRIVY] Limpando ficheiros temporários .tar ---"
	rm -f $(REPORTS_DIR)/users_temp.tar $(REPORTS_DIR)/products_temp.tar
	$(MAKE) fix-perms
	@echo "--- [RESULTADO] Conteúdo de $(REPORTS_DIR): ---"
	@ls -lh $(REPORTS_DIR)/trivy_*.txt
	@echo "Relatórios Trivy gerados com sucesso."
...
docker-run:
	...
	-docker compose exec -T users-service python3 -m black app/ tests/
	-docker compose exec -T users-service python3 -m flake8 app/ tests/ --max-line-length=120 > reports/docker/flake8_report.txt 2>&1
	-docker compose exec -T users-service python3 -m bandit -r app/ -ll > reports/docker/bandit_report.txt 2>&1
	-docker compose exec -T users-service python3 -m safety check --full-report > reports/docker/safety_report.txt 2>&1
	...
	-docker compose exec -T \
		...
		users-service python3 -m pytest -v tests/ > reports/docker/tests_report.txt 2>&1
	
	@echo "--- [STATUS] Resultados do Ciclo Docker ---"
	@# Verificamos se o ficheiro existe antes de dar cat para não quebrar o make se o teste falhar gravemente
	@if [ -f reports/docker/tests_report.txt ]; then cat reports/docker/tests_report.txt; else echo "Erro: Relatório de testes não foi gerado."; fi
...
docker-trivy-run:
	...
	@cp $(REPORTS_DIR)/trivy_report.txt $(REPORTS_DOCKER)/ 2>/dev/null || true
	@echo "--- [OK] Relatórios de teste Trivy em Docker copiados para $(REPORTS_DOCKER)/ ---"
...
docker-dashboard:
	...
	@sleep 40
	@echo "[STEP 1] Injetando carga real..."
	@echo "Injetando carga real... Este script gera 50 chamadas sequenciais"
	@echo "A gerar 50 requisições de teste. Verifique o Grafana (http://localhost:3000)."
	@# CICLO FOR INTEGRADO (As barras invertidas \ são obrigatórias para manter o contexto do loop)
	@for i in $$(seq 1 25); do \
			curl -s --connect-timeout 2 http://localhost:5000/users > /dev/null && echo "  - Microserviço Users: OK ($$i/25)"; \
			curl -s --connect-timeout 2 http://localhost:5001/products > /dev/null && echo "  - Microserviço Products: OK ($$i/25)"; \
			curl -s --connect-timeout 2 http://localhost:5001/e2e/products-with-users > /dev/null && echo "  - Fluxo E2E (Traces): OK ($$i/25)"; \
			sleep 0.1; \
	done
	@echo "Geração de tráfego concluída via Makefile."
	...
	@if curl -s -f http://localhost:9090/-/healthy > /dev/null; then \
		...
		echo "  -> [FAIL] Prometheus inacessível."; \
	fi

	@echo "\n[STEP 3] LOKI (porta 3100):"
	@# Teste via GET /ready evita o erro 405 que vimos anteriormente
	@if curl -s -f http://localhost:3100/ready > /dev/null; then \
		echo "  -> [OK] Loki Service: READY"; \
	else \
		echo "  -> [FAIL] Loki não respondeu READY."; \
	fi

	@echo "\n[STEP 4] TEMPO (porta 3200):"
	@# Agora que a porta está exposta no compose, o localhost resolve
	@if curl -s -f http://localhost:3200/ready > /dev/null; then \
		echo "  -> [OK] Tempo Service: READY"; \
	else \
		echo "  -> [FAIL] Tempo inacessível via localhost:3200."; \
	fi
